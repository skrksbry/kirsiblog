...
export const generateMetadata = async ({
	...
	return MetadataContent({
		...
		description: mdToMetaDescription(data.post_content),
		asPath: '',
		ogImage: `/og?id=${params.id}`,
	});
};

const MarkdownPostViewer = dynamic(
	() => import('@/components/view/MarkdownPostViewer'),
	{
		ssr: false,
			...
			<div className="blg-page-content-area blg-layout-width">
				<div className="flex w-full flex-1 relative flex-wrap content-start">
					<div className="flex w-full flex-wrap left-0 top-[0px] relative post-view">
						<h1 className="w-full relative text-4xl font-bold text-black dark:text-white">
							{postContent.post_name}
						</h1>
						<div className="flex-1 flex relative items-center text-gray-400 mt-6 gap-2">
							...
							<span className="text-md">
								{getDate(
									postContent.post_date
								)}
							</span>
						</div>
						<div className="flex relative items-center text-gray-400 mt-6 gap-2 ">
							<EyeIcon className="w-4 h-4" />
							{
								postContent.post_view_count
							}{' '}
					...
					>
						<div
							className="h-auto flex image-cover rounded-[12px] max-h-[300px]"
							style={{
								width: '100%',
								background: postContent.post_color,
								position: 'relative',
							}}
						>
							<Image
								src={
									postContent.post_image
								}
								alt={
									'타이틀 이미지'
								}
					...
					<MarkdownPostViewer
						md={postContent.post_content}
					/>
					<div className="py-4 mt-4 border-t-2 border-b-0 w-full flex border-dotted border-[#808080] border-opacity-30 flex-wrap">
						<span className="w-full font-extrabold text-xl px-2">
							Written by
						</span>
						<div
							className="bg-white rounded-full w-24 h-24 overflow-hidden border-[2px] pl-1.5 border-opacity-30 m-2"
							style={{
								background: postContent.post_color,
							}}
						>
